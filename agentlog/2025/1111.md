...
<details>
<summary>Design a cross-engine quantization parameter configuration scheme with SGLang priority</summary>
...
**Prompt**: "All quantization type fields you defined must be orthogonal to each other, and options within each field must be mutually exclusive. Reorganize the solution document, write only one."
...
**Prompt**: "Don't include offline weight quantization types, as those are determined by model configuration. GEMM dtype should have an independent field, right? Also, still need to consider compatibility across the three engines, provide a new solution."
