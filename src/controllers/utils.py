...
def parse_parallel_config(parameters: Dict[str, Any]) -> Dict[str, int]:
	...
	def safe_int_conversion(value, param_name, default=1):
		...
		try:
			...
			print(
				f"[parse_parallel_config] Warning: Failed to convert {param_name}={value} to int: {e}, using default={default}"
			)
			return default
	...
	return {"tp": tp, "pp": pp, "dp": dp, "cp": cp, "dcp": dcp, "world_size": world_size}
...
def setup_proxy_environment(
	base_env: Dict[str, str], http_proxy: str = "", https_proxy: str = "", no_proxy: str = "", hf_token: str = ""
) -> Dict[str, str]:
	"""
...
def get_runtime_config(runtime_name: str, image_tag: Optional[str] = None) -> Optional[Dict[str, str]]:
	"""
