...
class ToolsRegistry:
	...
	def get_safe_tools(self) -> List[BaseTool]:
		...
		return [t for t in self._tools.values() if not getattr(t, '_requires_authorization', False)]

	def get_privileged_tools(self) -> List[BaseTool]:
		"""Get tools that require authorization."""
		return [t for t in self._tools.values() if getattr(t, '_requires_authorization', False)]
