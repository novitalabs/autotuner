...
async def get_huggingface_model_info(model_id: str) -> str:
	...
	try:
		...
		return json.dumps(
			{"error": f"HTTP {e.response.status_code}: Model not found or API error", "model_id": model_id}
		)
	except Exception as e:
		return json.dumps({"error": f"Failed to fetch model info: {str(e)}", "model_id": model_id})


@tool
@register_tool(ToolCategory.API)
async def search_huggingface_models(query: str, limit: int = 5, filter_tag: str = None) -> str:
	"""
...
async def check_service_health(url: str) -> str:
	...
	try:
		...
		return json.dumps({"url": url, "error": "Request timed out after 5 seconds", "healthy": False})
	except httpx.ConnectError:
		return json.dumps({"url": url, "error": "Connection refused - service not reachable", "healthy": False})
	except Exception as e:
