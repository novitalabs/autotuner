...
async def create_preset(
	...
	db_preset = ParameterPreset(
		name=name, description=description, category=category, runtime=runtime, parameters=parameters, is_system=False
	)
	...
	return json.dumps({"success": True, "preset": db_preset.to_dict()}, indent=2)
...
async def update_preset(
	...
	if name is not None:
		...
		result = await db.execute(
			select(ParameterPreset).where(ParameterPreset.name == name, ParameterPreset.id != preset_id)
		)
	...
	return json.dumps(
		{"success": True, "message": f"Preset {preset_id} updated successfully", "preset": preset.to_dict()}, indent=2
	)
...
async def delete_preset(preset_id: int, db: AsyncSession = None) -> str:
	...
	return json.dumps({"success": True, "message": f"Preset '{preset_name}' (ID: {preset_id}) deleted successfully"})
