...
def _run_hf_command(args: list, timeout: int = 300) -> dict:
	...
	try:
		result = subprocess.run(["hf"] + args, capture_output=True, text=True, timeout=timeout)
		return {
...
def hf_cache_scan() -> str:
	...
	for line in lines:
		...
		if len(parts) >= 3:
			...
			if "/" in parts[0]:
				models.append({"model_id": parts[0], "size": parts[2] if len(parts) > 2 else "unknown"})
...
def hf_download_model(
	...
	if not result["success"]:
		return json.dumps({"success": False, "repo_id": repo_id, "error": result["stderr"]}, indent=2)
...
def hf_env_info() -> str:
	...
	return json.dumps({"success": True, "environment": env_info}, indent=2)
...
def hf_auth_status() -> str:
	...
	if not result["success"]:
		...
		if "not logged in" in result["stderr"].lower() or result["returncode"] == 1:
			return json.dumps(
				{"success": True, "logged_in": False, "message": "Not logged in to HuggingFace"}, indent=2
			)
		return json.dumps({"success": False, "error": result["stderr"]}, indent=2)

	username = result["stdout"].strip()
	return json.dumps(
		{"success": True, "logged_in": True, "username": username, "message": f"Logged in as {username}"}, indent=2
	)
