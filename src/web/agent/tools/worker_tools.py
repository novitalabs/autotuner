...
async def get_arq_worker_status() -> str:
	...
	try:
		...
		result = subprocess.run(["pgrep", "-af", "arq.*autotuner_worker"], capture_output=True, text=True, timeout=10)

		if result.returncode != 0 or not result.stdout.strip():
			return json.dumps(
				{"success": True, "status": "stopped", "workers": [], "message": "No ARQ worker processes found"},
				indent=2,
			)
		...
		return json.dumps({"success": False, "error": "Timeout while checking worker status"})
	except Exception as e:
		return json.dumps({"success": False, "error": f"Failed to check worker status: {str(e)}"})
...
async def restart_arq_worker() -> str:
	...
	try:
		...
		kill_result = subprocess.run(
			["pkill", "-f", "arq.*autotuner_worker"], capture_output=True, text=True, timeout=10
		)

		# Check if any workers are still running (might be owned by different user)
		check_result = subprocess.run(
			["pgrep", "-f", "arq.*autotuner_worker"], capture_output=True, text=True, timeout=5
		)
		...
		if check_result.returncode == 0 and check_result.stdout.strip():
			...
			for pid in pids:
				if pid.strip():
					subprocess.run(["sudo", "-n", "kill", "-9", pid.strip()], capture_output=True, text=True, timeout=5)
		...
		return json.dumps({"success": False, "error": "Timeout during worker restart"})
	except Exception as e:
		return json.dumps({"success": False, "error": f"Failed to restart worker: {str(e)}"})
...
async def list_arq_jobs() -> str:
	...
	try:
		...
		for key in result_keys[:5]:  # Last 5 results
			...
			if result_data:
				recent_results.append({"job_id": job_id, "has_result": True})
		...
		return json.dumps({"success": False, "error": f"Failed to list ARQ jobs: {str(e)}"})
