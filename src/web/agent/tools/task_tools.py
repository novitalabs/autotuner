...
async def create_task(
	...
	db_task = Task(
		...
		optimization_config={"strategy": optimization_strategy, "objective": optimization_objective},
		benchmark_config={
			"task": benchmark_task,
			"model_name": model_id,
	...
	return json.dumps({"success": True, "task": db_task.to_dict(include_full_config=True)}, indent=2)
...
async def delete_task(task_id: int, db: AsyncSession = None) -> str:
	...
	if task.status == TaskStatus.RUNNING:
		return json.dumps({"error": "Cannot delete running task. Cancel it first."})
...
async def clear_task_data(task_id: int, db: AsyncSession = None) -> str:
	...
	if task.status == TaskStatus.RUNNING:
		return json.dumps({"error": "Cannot clear running task. Cancel it first."})
...
@tool
@register_tool(ToolCategory.TASK)
async def update_task_description(task_id: int, description: str, db: AsyncSession = None) -> str:
	"""
...
async def list_tasks(
	skip: int = 0, limit: int = 20, status_filter: Optional[str] = None, db: AsyncSession = None
) -> str:
	"""
...
@tool
@register_tool(ToolCategory.TASK)
async def get_task_by_id(task_id: int, db: AsyncSession = None) -> str:
	"""
	...
	return json.dumps(
		{"success": True, "task": task.to_dict(include_full_config=True), "experiment_count": len(experiments)},
		indent=2,
	)
...
@tool
@register_tool(ToolCategory.TASK)
async def get_task_by_name(task_name: str, db: AsyncSession = None) -> str:
	"""
...
async def list_task_experiments(
	task_id: int, skip: int = 0, limit: int = 50, status_filter: Optional[str] = None, db: AsyncSession = None
) -> str:
	"""
	...
	experiments = await ExperimentService.list_experiments(
		db, task_id=task_id, status=status_filter, skip=skip, limit=limit
	)
...
@tool
@register_tool(ToolCategory.TASK)
async def get_task_results(task_id: int, include_all_experiments: bool = False, db: AsyncSession = None) -> str:
	"""
...
@tool
@register_tool(ToolCategory.TASK)
async def get_task_logs(task_id: int, tail_lines: Optional[int] = None, db: AsyncSession = None) -> str:
	"""
...
async def get_experiment_logs(
	task_id: int, experiment_id: int, tail_lines: Optional[int] = None, db: AsyncSession = None
) -> str:
	"""
		...
		return json.dumps(
			{
				...
				"hint": f"Available experiment IDs for task {task_id}: {available_exp_ids}"
				if available_exp_ids
				else "No experiment logs found for this task",
			},
			indent=2,
		)
...
@tool
@register_tool(ToolCategory.TASK)
async def get_experiment_details(experiment_id: int, db: AsyncSession = None) -> str:
	"""
...
@tool
@register_tool(ToolCategory.TASK)
async def analyze_slo_violations(task_id: int, db: AsyncSession = None) -> str:
	"""
	...
	if not experiments:
		return json.dumps(
			{"success": True, "task_id": task_id, "has_slo": True, "message": "No experiments found for this task"}
		)
		...
		if "ttft" in slo_config:
			...
			if threshold and metrics.get("ttft") is not None:
				...
				if actual > threshold:
					...
					violation_stats["metric_violations"]["ttft"] = (
						violation_stats["metric_violations"].get("ttft", 0) + 1
					)
		...
		if "tpot" in slo_config:
			...
			if threshold and metrics.get("tpot") is not None:
				...
				if actual > threshold:
					...
					violation_stats["metric_violations"]["tpot"] = (
						violation_stats["metric_violations"].get("tpot", 0) + 1
					)
				...
				if percentile in slo_config["latency"]:
					...
					if threshold and metrics.get(metric_key) is not None:
						...
						if actual > threshold:
							...
							violation_stats["metric_violations"][metric_key] = (
								violation_stats["metric_violations"].get(metric_key, 0) + 1
							)
		...
		if "throughput" in slo_config:
			...
			if threshold and metrics.get("throughput") is not None:
				...
				if actual < threshold:
					...
					violation_stats["metric_violations"]["throughput"] = (
						violation_stats["metric_violations"].get("throughput", 0) + 1
					)
	...
	most_violated = sorted(violation_stats["metric_violations"].items(), key=lambda x: x[1], reverse=True)
...
async def search_experiment_logs(
	task_id: int, experiment_id: int, context_lines: int = 10, db: AsyncSession = None
) -> str:
	"""
	...
	if not log_file.exists():
		return json.dumps({"success": True, "log_exists": False, "message": f"No log file found for task {task_id}"})
				...
				matching_sections.append(
					{
						"line_number": i + 1,
						"matching_line": line,
						"context": {"before": lines[start:i], "after": lines[i + 1 : end]},
					}
				)
