...
async def query_records(table_name: str, filters: dict = None, limit: int = 10, db: AsyncSession = None) -> str:
	"""
	...
	if table_name not in valid_tables:
		return json.dumps({"error": f"Invalid table name. Must be one of: {', '.join(valid_tables)}"})
	...
	try:
		...
		if rows:
			...
			return json.dumps(
				{"success": True, "table": table_name, "row_count": len(results), "results": results}, indent=2
			)
		else:
			return json.dumps({"success": True, "table": table_name, "row_count": 0, "results": []})
...
@tool
@register_tool(ToolCategory.DATABASE)
async def get_record_by_id(table_name: str, record_id: int, db: AsyncSession = None) -> str:
	"""
	...
	try:
		...
		if row:
			...
			return json.dumps({"success": True, "table": table_name, "record": record}, indent=2)
		else:
			return json.dumps({"error": f"No record found with id {record_id} in table {table_name}"})
...
@tool
@register_tool(ToolCategory.DATABASE, requires_auth=True, auth_scope=AuthorizationScope.DATABASE_WRITE)
async def update_database_field(
	table_name: str, record_id: int, field_name: str, field_value: str, db: AsyncSession = None
) -> str:
	"""
	...
	**REQUIRES AUTHORIZATION**: This is a privileged operation that requires user approval.
	...
	try:
		...
		await db.rollback()
		return json.dumps({"error": f"Failed to update record: {str(e)}"})


@tool
@register_tool(ToolCategory.DATABASE, requires_auth=True, auth_scope=AuthorizationScope.DATABASE_WRITE)
async def execute_raw_sql(sql_query: str, is_write: bool = False, db: AsyncSession = None) -> str:
	"""
	...
	**DANGEROUS**: Use with caution. Prefer high-level business tools when available.
	...
	for keyword in dangerous_keywords:
		if keyword in query_upper:
			return json.dumps({"error": f"Dangerous operation '{keyword}' not allowed. Use high-level tools instead."})
	...
	try:
		...
		if is_write:
			await db.commit()
			return json.dumps(
				{"success": True, "message": "Query executed successfully", "rows_affected": result.rowcount}, indent=2
			)
		else:
			...
			if rows:
				...
				columns = result.keys()
				results = [dict(zip(columns, row)) for row in rows]
				return json.dumps({"success": True, "row_count": len(results), "results": results}, indent=2)
			else:
				return json.dumps({"success": True, "row_count": 0, "results": []})
