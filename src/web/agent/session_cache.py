...
class SessionCache:
	...
	def __init__(self, maxsize: int = 100, ttl_seconds: int = 86400):
		...
		logger.info(f"Session cache initialized: maxsize={maxsize}, ttl={ttl_seconds}s ({ttl_seconds/3600:.1f}h)")
	...
	def set(self, session_id: str, messages: List[Dict]) -> None:
		...
		logger.debug(f"Cached session {session_id} with {len(messages)} messages")
