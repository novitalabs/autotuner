...
class GPUResourcePool:
	...
	async def __aexit__(self, exc_type, exc_val, exc_tb):
		...
		async with self._lock:
			if self._allocations:
				logger.warning(f"GPUResourcePool cleanup: {len(self._allocations)} " f"allocations still active")
	...
	async def acquire(
		...
		logger.info(
			f"Requesting {required_gpus} GPUs (min_memory={min_memory_mb}MB) " f"for experiment {experiment_id}"
		)

		try:
			if timeout:
				return await asyncio.wait_for(
					self._acquire_internal(required_gpus, min_memory_mb, experiment_id, params), timeout=timeout
				)
			else:
				return await self._acquire_internal(required_gpus, min_memory_mb, experiment_id, params)
		except asyncio.TimeoutError:
			elapsed = asyncio.get_event_loop().time() - start_time
			logger.error(f"GPU acquisition timeout after {elapsed:.1f}s " f"(requested {required_gpus} GPUs)")
			raise

	async def _acquire_internal(
		self, required_gpus: int, min_memory_mb: int, experiment_id: Optional[int], params: Optional[dict]
	) -> GPUAllocation:
		...
		while True:
			...
			logger.debug(f"At capacity ({len(self._allocations)}/{self.max_parallel}), " f"waiting for slot...")
		...
		async with self._lock:
			...
			allocation = GPUAllocation(
				gpu_indices=gpu_indices, allocated_at=datetime.now(), experiment_id=experiment_id, params=params
			)
	...
	async def _select_gpus(self, required_gpus: int, min_memory_mb: int) -> List[int]:
		"""
		...
		if not self._gpu_monitor.is_available():
			logger.warning("GPU monitoring not available, returning sequential GPU indices")
		...
		for gpu in snapshot.gpus:
			...
			if gpu.memory_free_mb < min_memory_mb:
				logger.debug(
					f"GPU {gpu.index} has insufficient memory " f"({gpu.memory_free_mb}MB < {min_memory_mb}MB)"
				)
				continue
		...
		logger.debug(f"Selected GPUs {gpu_indices} " f"(scores: {[f'{g.score:.3f}' for g in selected]})")
	...
	async def _release_internal(self, allocation: GPUAllocation) -> None:
		...
		if allocation in self._allocations:
			...
			logger.warning(f"Attempted to release unknown allocation: {allocation}")
			...
			"allocations": [
				{
					...
					"duration_seconds": (datetime.now() - alloc.allocated_at).total_seconds(),
				}
...
async def estimate_and_acquire(
	...
	logger.info(f"Estimated requirements: {required_gpus} GPUs, " f"{estimated_memory_mb}MB per GPU")
