...
class DatasetManager:
	...
	def get_dataset(
		...
		output_file = self._convert_to_genai_format(data_file, converted_dir, deduplicate=deduplicate)
	...
	def _extract_archive(self, archive_path: Path, extract_dir: Path) -> Path:
		...
		if suffix == ".gz" and not name.endswith(".tar.gz"):
			...
			raise ValueError(f"Unsupported file format: {suffix}. " f"Supported: .csv, .jsonl, .gz, .zip, .tar.gz")
	...
	def _find_data_file(self, directory: Path) -> Path:
		...
		raise ValueError(f"No CSV or JSONL file found in extracted archive. " f"Contents: {list(directory.rglob('*'))}")
	...
	def _parse_csv_logs(self, csv_file: Path) -> list[str]:
		...
		if "request_body" not in df.columns:
			raise ValueError(f"CSV must have 'request_body' column. " f"Found columns: {list(df.columns)}")
			...
			for line in f:
				...
				try:
					...
					# Try request_body JSON
						...
						prompt = self._extract_prompt_from_request_body(data["request_body"])
						if prompt:
							prompts.append(prompt)
						else:
							parse_errors += 1
