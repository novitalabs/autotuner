...
def find_available_ports(count: int, start_port: int, end_port: int, host: str = "0.0.0.0") -> list[int]:
	"""
	...
	if len(available_ports) < count:
		logger.warning("Only found %d available ports, needed %d", len(available_ports), count)
...
def check_port_connectivity(host: str, port: int, timeout: float = 2.0) -> tuple[bool, Optional[str]]:
	"""
