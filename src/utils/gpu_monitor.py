...
class GPUMonitor:
	...
	def _query_gpu_processes(self, gpu_index: int) -> List[Dict[str, Any]]:
		...
		try:
			...
			for line in result.stdout.strip().split("\n"):
				...
				parts = [p.strip() for p in line.split(",")]
				if len(parts) >= 2:
					processes.append({"pid": int(parts[0]), "used_memory_mb": int(parts[1])})
	...
	def get_available_gpus(self, min_memory_mb: Optional[int] = None, max_utilization: int = 50) -> List[int]:
		"""
	...
	def allocate_gpus(self, count: int, min_memory_mb: Optional[int] = None) -> Tuple[List[int], bool]:
		"""
		...
		if len(available) < count:
			logger.warning(
				f"Requested {count} GPUs but only {len(available)} available " f"(min_memory_mb={min_memory_mb})"
			)
			return ([], False)
		...
		if count > 1 and min_memory_mb:
			...
			if snapshot:
				...
				if allocated_gpu_info:
					...
					if not is_balanced:
						logger.error(
							f"GPU memory imbalance detected: {msg}. " f"Rejecting allocation due to memory imbalance."
						)
						return ([], False)
	...
	def monitor_gpus(
		self, gpu_indices: List[int], duration_seconds: float, interval_seconds: float = 1.0
	) -> List[GPUSnapshot]:
		"""
		...
		while (time.time() - start_time) < duration_seconds:
			...
			if snapshot:
				...
				filtered_snapshot = GPUSnapshot(
					...
					available_gpus=sum(
						1 for gpu in filtered_gpus if gpu.utilization_gpu < 50 and gpu.utilization_memory < 80
					),
				)
